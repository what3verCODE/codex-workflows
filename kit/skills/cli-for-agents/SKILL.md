...
description: >-
  ...
  adding commands, writing --help, or reviewing whether a CLI can run unattended.
---
...
- Missing required values fail with an actionable error in unattended mode. A human-facing interactive mode may prompt when explicitly selected or when the CLI's documented terminal behavior allows it. Every prompt still has an equivalent flag.
