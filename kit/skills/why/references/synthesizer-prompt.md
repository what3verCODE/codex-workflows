...
Write the output for the user. Use the following structure for a substantial investigation. For a small question, combine sections while retaining evidence, confidence and actual coverage.
...
- **Source control history**: {file paths}, {number of commits reviewed}, PRs #{numbers}, and code comments searched. Or "Unavailable or not searched, with the actual reason."
...
4. Does the "What We Don't Know" section exist and name specific gaps? Report actual gaps when present; do not invent missing evidence merely to populate a section.
...
Revise unsupported claims before returning. Use only available source tools; these examples do not imply access or authorization.
