...
Once a variant has won, capture the answer (which variant and why), then capture the prototype the way the [SKILL](SKILL.md) describes. When production integration is authorized, fold the winner into the real code and preserve the alternatives in the prototype artifact location:

- **Sub-shape A**: fold the winner into the existing page; drop the losing variants and the switcher from the production deliverable.
- **Sub-shape B**: promote the winning variant to a real route; drop the throwaway route and the switcher from the production deliverable.

Preserve the full set of variants as a runnable primary source using the artifact and VCS guidance in SKILL.md. Keep variants and the switcher outside the production deliverable.
